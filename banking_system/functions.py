...
def add_transaction(transaction_type, amount,source_account_id,destination_account_id = None):
    ...
    while loop_status:
        ...
        if status := check_locked(accounts_list):
            sleep(0.5)
        else:
            loop_status = False
...
def locked_account(accounts = None):
    if accounts is None:
        accounts = []
...
def unlocked_account(accounts = None):
    if accounts is None:
        accounts = []
...
def check_locked(accounts = None):
    if accounts is None:
        accounts = []
